...
VariableChangeFactory::VariableChangeFactory(const std::string & name) {
  if (getMakers().find(name) != getMakers().end()) {
    oops::Log::error() << name
                        << " already registered in wrf_hydro_nwm_jedi::VariableChangeFactory."
                       << std::endl;
    ABORT("Element already registered in wrf_hydro_nwm_jedi::VariableChangeFactory.");
  }
  getMakers()[name] = this;
}
...
VariableChangeBase * VariableChangeFactory::create(const Geometry & geom,
  ...
  if (jloc == getMakers().end()) {
    oops::Log::error() << id
              << " does not exist in wrf_hydro_nwm_jedi::VariableChangeFactory." << std::endl;
    ABORT("Element does not exist in wrf_hydro_nwm_jedi::VariableChangeFactory.");
  }
